...
describe('JsonWorldGenerator', () => {
    ...
    it('should handle a large world with multiple entity types', () => {
        ...
        entityTypes.forEach(componentTypes => {
            expect(countEntitiesByComponents(importedWorld, componentTypes))
                .toBe(countEntitiesByComponents(originalWorld, componentTypes));
        });
        ...
        [[0, 0], [0, worldHeight-1], [worldWidth-1, 0], [worldWidth-1, worldHeight-1]].forEach(([x, y]) => {
            const originalEntities = getEntityAt(originalWorld, x, y);
            const importedEntities = getEntityAt(importedWorld, x, y);
            expect(importedEntities.length).toBe(originalEntities.length);
            
            // Check that it's a wall (has all three components)
            const importedWall = importedEntities.find(e => 
                Array.from(e.getComponents()).length === 3
            );
